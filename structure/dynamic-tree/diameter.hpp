...
template <typename T>
struct Diameter {
  static constexpr T m_inf = std::numeric_limits<T>::min();
  struct Point {
    T dia, dep1, dep2;
  };
  struct Path {
  ...
  static Path add_vertex(const Point& d, const Info& u) {
    ...
    if (d.dep1 != m_inf) {
      ...
      if (d.dep2 != m_inf) {
        p.dia = std::max(p.dia, d.dep1 + u.w + d.dep2);
      }
    }
    if (u.is_vertex) {
      p.pref_max = std::max(p.pref_max, u.w);
      p.dia = std::max(p.dia, p.pref_max);
    }
  ...
  static Point rake(Point l, const Point& r) {
    l.dia = std::max(l.dia, r.dia);
    if (l.dep1 < r.dep1) {
      l.dep2 = std::max(l.dep1, r.dep2);
      l.dep1 = r.dep1;
    } else {
      l.dep2 = std::max(l.dep2, r.dep1);
    }
    return l;
  }
  static Path compress(const Path& p, const Path& c) {
    Path t;
    t.dia = std::max(p.dia, c.dia);
    if (p.suf_max != m_inf and c.pref_max != m_inf) {
      t.dia = std::max(t.dia, p.suf_max + c.pref_max);
    }
    t.all = p.all + c.all;
    t.pref_max = p.pref_max;
    t.suf_max = c.suf_max;
    if (c.pref_max != m_inf) {
      t.pref_max = std::max(t.pref_max, p.all + c.pref_max);
    }
    if (p.suf_max != m_inf) {
      t.suf_max = std::max(t.suf_max, c.all + p.suf_max);
    }
    return t;
  }
};
