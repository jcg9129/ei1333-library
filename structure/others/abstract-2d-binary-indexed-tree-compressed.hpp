...
struct Abstract2DBinaryIndexedTreeCompressed {
 private:
  int n;
  std::vector<AbstractBinaryIndexedTree<T, F> > data;
  const F f;
  const T e;
  std::vector<int> hs;
  std::vector<std::vector<int> > beet;

 public:
  Abstract2DBinaryIndexedTreeCompressed(const std::vector<int>& hs, const F f,
                                        const T& e)
    ...
    data.reserve(n + 1);
    for (int k = 0; k <= n; k++) {
      beet[k].erase(std::unique(std::begin(beet[k]), std::end(beet[k])),
                    std::end(beet[k]));
  ...
  void apply(int k1, const T& x) {
    ...
    for (++k1; k1 <= n; k1 += k1 & -k1) {
      int p = std::lower_bound(std::begin(beet[k1]), std::end(beet[k1]), k2) -
              std::begin(beet[k1]);
      data[k1].apply(p, x);
    }
  }
  ...
  T prod(int r1, int r2) const {
    T ret{e};
    for (; r1 > 0; r1 -= r1 & -r1) {
      int p = std::lower_bound(std::begin(beet[r1]), std::end(beet[r1]), r2) -
              std::begin(beet[r1]);
...
get_abstract_2d_binary_indexed_tree_compressed(const std::vector<int>& hs,
                                               const F& f, const T& e) {
