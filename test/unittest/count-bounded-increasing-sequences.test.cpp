...
mint naive(const vector<int> &lower_bounds, const vector<int> &upper_bounds) {
  ...
  vector<mint> dp(*max_element(upper_bounds.begin(), upper_bounds.end()));
...
void check(const vector<int> &lower_bounds, const vector<int> &upper_bounds) {
  auto got =
      count_bounded_increasing_sequences<mint>(lower_bounds, upper_bounds);
...
int main() {
  ...
  check({0}, {0});
  check({0}, {1});
  assert(count_bounded_increasing_sequences<mint>({0, 0}, {2, 2}) == 3);
  assert(count_bounded_increasing_sequences<mint>({1, 0}, {2, 2}) == 1);
  check({0}, {10});
  ...
  vector<int> large_upper(80);
  iota(large_upper.begin(), large_upper.end(), 40);
  check(vector<int>(80), large_upper);
