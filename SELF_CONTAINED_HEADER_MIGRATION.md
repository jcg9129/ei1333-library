...
import と別名を意図的に提供する集約ヘッダーのため、自己完結性検査から
除外している。
...
CI で一律に検査する。検査対象は Git の追跡ファイルから自動的に取得する。
...
include sorting を有効にした clang-format 対象としている。
...
5. 正常系、境界値、重要な前提を検査する単体テストを追加または更新する
6. 関連ドキュメントのシグネチャや制約を更新する
7. clang-format、ヘッダー単体コンパイル、単体テスト、docs check を実行する
8. この文書の進捗件数と移行履歴を更新する
...
| 2026-08-10 | 移行用一覧ファイル2件 | 一律検査への移行完了に伴い削除 |
