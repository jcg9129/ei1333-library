...
各頂点の DFS における訪問順 `ord` と、DFS 木の部分木から到達できる訪問中の頂点の最小訪問順 `low` を求める。`ord` と `low` が等しい頂点を根とする部分が $1$ つの強連結成分となる。DFS は明示的なスタックを用いて非再帰で実行するため、グラフが深い場合もコールスタックを消費しない。また、逆辺グラフは構築しない。
