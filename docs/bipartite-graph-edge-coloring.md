---
title: Bipartite Graph Edge Coloring (二部グラフの辺彩色)
...
$a$ から $b$ に辺を張る。$a$ は二部グラフの左側、$b$ は右側の頂点を指す。
...
二部グラフの辺彩色を返す。同じ色に塗るべき辺の番号が同じ配列に格納される。辺の番号は `add_edge()` を呼び出した順に 0-indexed で付与される。
